...
void delay_init()
{
	...
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM5, ENABLE);           //使能TIM1时钟	
	                                          //定时器1寄存器恢复默认值
	TIM_TimeBaseInitStructure.TIM_Period = 1000-1; 	          //设置自动重装载值
	TIM_TimeBaseInitStructure.TIM_Prescaler=72-1;              //设置定时器预分频数
	TIM_TimeBaseInitStructure.TIM_RepetitionCounter=0;          //设置定时器重复计数2次（TIM1和8 才有这个功能）
	TIM_TimeBaseInitStructure.TIM_CounterMode=TIM_CounterMode_Down; //向上计数模式
	TIM_TimeBaseInitStructure.TIM_ClockDivision=TIM_CKD_DIV1;     //1分频
	TIM_TimeBaseInit(TIM5,&TIM_TimeBaseInitStructure);            //设置TIM1
		
  while((TIM5->SR & TIM_FLAG_Update)!=SET);                                           // 计时到时，此处为1
