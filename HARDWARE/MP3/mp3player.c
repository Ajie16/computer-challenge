...
void mp3_play(void)
{
  ...
  res=f_opendir(&mp3dir,"0:/MUSIC"); //打开目录
   ...
   res=f_opendir(&mp3dir,(const TCHAR*)"0:/MUSIC"); 	//打开目录
	...
	{	
		LCD_Fill(50,130,170,190+16,WHITE);				//清除之前的显示 
		
		for(i=curindex+1;i<curindex+3;i++)
		{
			if(i>=totmp3num)
				dir_sdi(&mp3dir,mp3indextbl[i-totmp3num]);
			else
				dir_sdi(&mp3dir,mp3indextbl[i]);			//改变当前目录索引	   
			res=f_readdir(&mp3dir,&mp3fileinfo);       		//读取目录下的一个文件
			fn=(u8*)(*mp3fileinfo.lfname?mp3fileinfo.lfname:mp3fileinfo.fname);		
			strcpy((char*)pname,"0:/MUSIC/");				//复制路径(目录)
			strcat((char*)pname,(const char*)fn);  			//将文件名接在后面
			pos_y+=20;
			Show_Str(50,pos_y,150,16,fn,16,0);				//显示歌曲名字 
		}
		
		if(curindex==0)
			dir_sdi(&mp3dir,mp3indextbl[totmp3num-1]);	
		else
			dir_sdi(&mp3dir,mp3indextbl[curindex-1]);			//改变当前目录索引	   
		res=f_readdir(&mp3dir,&mp3fileinfo);       		//读取目录下的一个文件
		fn=(u8*)(*mp3fileinfo.lfname?mp3fileinfo.lfname:mp3fileinfo.fname);		
		strcpy((char*)pname,"0:/MUSIC/");				//复制路径(目录)
		strcat((char*)pname,(const char*)fn);  			//将文件名接在后面
    ...
    if(res!=FR_OK||mp3fileinfo.fname[0]==0)break;	//错误了/到末尾了,退出
		...
		strcat((char*)pname,(const char*)fn);  			//将文件名接在后面
		pos_y=150;
		Show_Str(30,pos_y,16,16,">>",16,0);				//显示歌曲名字 
		Show_Str(50,pos_y,150,16,fn,16,0);				//显示歌曲名字
		...
		if(key==2)		//上一曲
		{
			...
			{
				seed_cnt=Math_Rand();
				curindex=(u16)seed_cnt%totmp3num;
			}
 		}else if(key<=1)//下一曲
		{
			if(mode==0)//顺序播放
...
{	 
  ...
  u16 br;
	...
	OS_ERR err;
	OS_FLAGS flags_rdy1,flags_rdy2,flags_rdy3,flags_rdy4;   
		...
		{ 
			...
			while(rval==0)
			{
                ...
                (OS_ERR      *)&err);
							...
							flags_rdy4 =  OSFlagPend ((OS_FLAG_GRP *)&flag_grp1,                 
													(OS_FLAGS     )( KEY8_EVENT ),
													(OS_TICK      )0,
													(OS_OPT       )OS_OPT_PEND_FLAG_SET_ALL |
																				 OS_OPT_PEND_NON_BLOCKING |
							...
							if((flags_rdy1 & (KEY1_EVENT)) == (KEY1_EVENT))
							{
								rval=1;		//下一曲
							}
							if((flags_rdy2 & (KEY2_EVENT)) == (KEY2_EVENT))
							{
								rval=2;		//上一曲
							}
							if((flags_rdy3 & (KEY4_EVENT)) == (KEY4_EVENT))
							{
								if(vsset.mvol<250)
								{
									vsset.mvol+=5;
							...
							if((flags_rdy4 & (KEY8_EVENT)) == (KEY8_EVENT))
							{
								if(vsset.mvol>100)
								{
									vsset.mvol-=5;
