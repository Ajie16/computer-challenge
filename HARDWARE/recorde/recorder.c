...
{   
	//显示录音时间			 
	LCD_ShowString(270,250,200,16,16,"TIME:");	  	  
	LCD_ShowxNum(270+40,250,tsec/60,2,16,0X80);	//分钟
	LCD_ShowChar(270+56,250,':',16,0);
	LCD_ShowxNum(270+64,250,tsec%60,2,16,0X80);	//秒钟		
}  	   
//通过时间获取文件名
...
{	 
	...
	while(index<0XFFFF)
	{
		sprintf((char*)pname,"0:RECORDER/录音%03d.wav",index);
...
{  
	LCD_ShowString(250+110,250,64,16,16,"AGC:    ");	  	//显示名称,同时清楚上次的显示	  
	if(agc==0)LCD_ShowString(250+142,250,32,16,16,"AUTO");	//自动agc	  	  
	else LCD_ShowxNum(250+142,250,agc,2,16,0X80);			//显示AGC值	 
} 
  	...
  	recoder_show_time(recsec);				//显示时间
 	   	...
 	   	while(rval==0)
		{
                ...
                (OS_ERR      *)&err);
					...
					if((flags_rdy1 & (KEY14_EVENT)) == (KEY14_EVENT))
					{
							if(rec_sta&0X80)//有录音
							{
							...
							rec_sta=0;
							recsec=0;
							LCD_Fill(230,230,240,246,WHITE);	//清除显示,清除之前显示的录音文件名	   
							Show_Str(270,230,240,16,"文件已存储！",16,0);	//清除显示,清除之前显示的录音文件名	 	   					
							Show_Str(230+150,210,50,16,"停止",16,0);
							recoder_show_time(recsec);		//显示时间
							}
							else
							{
							  Show_Str(270,230,50,16,"播放:",16,0);		   
								Show_Str(270+40,230,240,16,pname+11,16,0);	//显示当播放的文件名字   
								
								rec_play_wav(pname);						//播放pname
								LCD_Fill(270,230,270+40+100,246,WHITE);	//清除显示,清除之前显示的录音文件名	  
								Show_Str(270,230,80,16,"播放完毕!",16,0);								
					...
					if((flags_rdy2 & (KEY15_EVENT)) == (KEY15_EVENT))
					{
							if(rec_sta&0X01)//原来是暂停,继续录音
							{
								rec_sta&=0XFE;//取消暂停
								Show_Str(230+150,210,50,16,"录音",16,0);
							}else if(rec_sta&0X80)//已经在录音了,暂停
							{
								rec_sta|=0X01;	//暂停
								Show_Str(230+150,210,50,16,"暂停",16,0);
							}else				//还没开始录音 
							{
								rec_sta|=0X80;	//开始录音	 	 
								recoder_new_pathname(pname);			//得到新的名字
								Show_Str(270,230,240,16,pname+11,16,0);	//显示当前录音文件名字
								recoder_wav_init(wavhead);				//初始化wav数据	
								res=f_open(f_rec,(const TCHAR*)pname, FA_CREATE_ALWAYS | FA_WRITE); 
								if(res)			//文件创建失败
								{
									rec_sta=0;	//创建文件失败,不能录音
									Show_Str(230+150,230,50,16,"WRONG",16,0);
									rval=0XFE;	//提示是否存在SD卡
